...
cli.responders.help = function () {
  let commands = {
    ...
    'list users':
      'Show a list with all the registered (undeleted) users in the system',
    ...
    if (!err && checkIds && checkIds.length > 0) {
      ...
      checkIds.forEach(function (checkId) {
        _data.read('checks', checkId, function (err, checkData) {
          ...
          // Get the sate default to unknown
          let stateOrUnknown =
            typeof checkData.state == 'string' ? checkData.state : 'unknown';

          // If the user has specified the state or hasn't specifed any state, include the current check accordingly
          if (
